...
func Node_action(dynamicContext *dynamic.DynamicClient, df dataframe.DataFrame, idx int) {
	...
	// Attempts to parse memory as a float for numerical values (e.g., "4096" for MB).
	...
	if memFloat, err := strconv.ParseFloat(memInput, 64); err == nil {
		// If it's a number (assumed to be in MB), convert it to MiB and format as "Mi".
		// E.g., "4096" MB becomes "4096Mi" MiB (approx. 4Gi).
		memRequested = fmt.Sprintf("%dMi", int64(memFloat))
	} else {
	...
	if err != nil {
		...
		return // Cannot proceed without the correct cluster context.
	}
